...
def warn(user: User, chat: Chat, reason: str, message: Message, warner: User = None, conn=False) -> str:
    ...
    if num_warns >= limit:
        ...
        if num_warns+1 == limit:
            ...
            reply = tl(chat.id, "❗️ <b>Warn Event</b>\n• <b>User:</b> {}\n<b>Count:</b> {}/{} peringatan... Jika anda di peringati lagi maka kamu akan di {}!").format(mention_html(user.id, user.first_name), num_warns, limit, action_mode)
        else:
            reply = tl(chat.id, "❗️ <b>Warn Event</b>\n• <b>User:</b> {}\n<b>Count:</b> {}/{} peringatan... Hati-hati!").format(mention_html(user.id, user.first_name), num_warns, limit)
        if reason:
