...
def slap(bot: Bot, update: Update, args: List[str]):
    ...
    #else:
    curr_user = f"{mention_markdown(msg.from_user.id, msg.from_user.first_name)}"
    ...
    if user_id := extract_user(update.effective_message, args):
        ...
        #else:
        user2 = f"{mention_markdown(slapped_user.id, slapped_user.first_name)}"

    else:
        user1 = f"{mention_markdown(bot.id, bot.first_name)}"
        user2 = curr_user
...
def info(bot: Bot, update: Update, args: List[str]):
    ...
    text = (
        tl(update.effective_message, "<b>Info Pengguna</b>:")
        ...
        if loc.get('status') == 'OK':
            ...
            if res.status_code == 200:
                ...
                send_message(
                    ...
                    f"Sekarang pukul {time_there} di {location}",
                )
