...
def markdown_parser(txt: str, entities: Dict[MessageEntity, str] = None, offset: int = 0) -> str:
    ...
    for ent, ent_text in entities.items():
        ...
        if ent.type not in ("bold", "italic", "code", "url", "text_link"):
            continue
        ...
        if ent.type == "url":
            ...
            res += f'{_selective_escape(txt[prev:start])}_{ent_text}_'
...
def extract_time(message, time_val):
    if any(time_val.endswith(unit) for unit in ('m', 'h', 'd')):
        ...
        message.reply_text(
            f"Jenis waktu tidak valid ditentukan. Diharapkan m, h, atau d, didapatkan: {time_val[-1]}"
        )
        return ""
